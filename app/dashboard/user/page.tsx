...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    // Role guard — only user allowed here
  ...
  if (loading) return (<>{BG}<div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><style>{`@keyframes bounce{0%,80%,100%{transform:scale(0)}40%{transform:scale(1)}}`}</style><div style={{ display: "flex", gap: 7 }}>{[0,1,2].map(i => <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--us)", animation: `bounce 1s ${i*0.15}s infinite`, opacity: 0.8 }} />)}</div></div></>);
